...
bool isLightQuark(int partonId){
    return abs(partonId)==1 or abs(partonId)==2 or abs(partonId)==3;
}

bool isGluon(int partonId){
    return partonId==21;
}


string make_dataset_step1(const char* input_path, string data_dir="../data/root_format"){
	...
	if(not(isLightQuark(partonId) or isGluon(partonId))) continue;

	if(axis2==-log(0.)) continue;
