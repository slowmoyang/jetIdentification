...
# arson
from arson.pipeline import JetVarDataset
...
def train(args):
    ...
    training_dataset = JetVarDataset(datasets['training'])
    validation_dataset = JetVarDataset(datasets['validation']) 
    ...
    model.cuda()
    # Define bce with classification loss and use it as the criterion.
    ...
    for epoch in range(args.num_epochs):
	    ...
	    optimizer.step()
            ...
            if(step % args.val_freq == 0):
                ...
                record = validate(
                    model=model, data_loader=val_loader, bce_loss=bce_loss
                )
                ...
                print('    BCE Loss: %.4f | ACC.: %.4f | AUC: %.4f' % (bce_loss_value.data[0], acc_value, auc_value))

                print('  On validation data: ')
                print('    BCE Loss: %.4f | ACC.: %.4f | AUC: %.4f\n' % (record['bce_loss'], record['acc'], record['auc']))
...
def validate(model, data_loader, bce_loss):
...
def main():
    ...
    parser.add_argument("--log_dir", type=str, default='full_vars_test_1')
    #
