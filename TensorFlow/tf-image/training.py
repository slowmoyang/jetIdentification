...
from turbulence.metrics import Meter 
...
def train(training_files,
    ...
    with tf.Graph().as_default():
        ...
        meter = Meter(
            data_name_list=["step", "tr_acc", "val_acc", "tr_loss", "val_loss"],
        ...
        meter.prepare(
            data_pair_list=[("step", "tr_loss"), ("step", "val_loss")],
            ...
            while True:
                try:
                    ...
                    if(step%args.validation_freq==0):
                        ...
                        validation_feed = {handle: validation_handle, is_training: False, dropout_rate: 1.0}
                        val_loss_value, val_acc_value = sess.run(
                            fetches=validation_fetches,
                            feed_dict=validation_feed
                        )

                        training_fetches = [merged, train_op, loss, accuracy]
                        training_feed = {handle: training_handle, is_training: True, dropout_rate: 0.5}
                        # Record execution stats
                        run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE)
                        run_metadata = tf.RunMetadata()
                        summary, _, tr_loss_value, tr_acc_value = sess.run(
                        ...
                        if(step%args.print_freq==0):
                            ...
                            print('  Training:')
                            print('    Loss %.3f | Acc. %.3f' % (tr_loss_value[0], tr_acc_value))
                            print('  Validation:')
                            print('    Loss %.3f | Acc. %.3f\n' % (val_loss_value[0], val_acc_value))

                        meter_dict = {
                            "step": step,
                            "tr_loss": tr_loss_value[0],
                            "val_loss": val_loss_value[0],
                            "tr_acc": tr_acc_value,
...
def main():
    ...
    parser.add_argument(
	'--log_dir', type=str,
	default='./logs/image-%s' % datetime.today().strftime("%Y-%m-%d_%H-%M-%S"),
	help='the directory path of dataset'
    )

    # hyperparameter
    parser.add_argument('--batch_size', type=int, default=128, help='')
    parser.add_argument('--val_batch_size', type=int, default=128, help='')
